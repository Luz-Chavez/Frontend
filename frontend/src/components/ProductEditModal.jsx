...
function ProductEditModal({ producto, categorias, onClose, onSave }) {
  const [form, setForm] = useState({ ...producto });
  const [error, setError] = useState("");

  // Validación básica
  const validate = () => {
    if (!form.nombre || !form.precio_venta || !form.id_categoria) {
  ...
  const handleSave = () => {
    if (validate()) {
      // Asegurar que los campos cumplen el contrato del backend
      const payload = {
        ...
        codigo: form.codigo,
        imagen: typeof form.imagen === "string" ? form.imagen : "",
        estado: form.estado === true || form.estado === "activo" ? true : false,
        id_categoria: Number(form.id_categoria),
        id_microempresa: form.id_microempresa
      };
      onSave(payload);
      onClose();
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: 0, fontSize: "1.3rem", fontWeight: 700 }}>Editar producto</h2>
        ...
        <label style={{ fontWeight: 600, color: "#0A3A40" }}>
          ...
          {form.imagen && (
            <div style={{ marginTop: "0.5em", textAlign: "center" }}>
              <img src={form.imagen} alt="Previsualización" style={{ maxWidth: 120, maxHeight: 120, borderRadius: 8, boxShadow: "0 2px 8px 0 rgba(10,58,64,0.10)" }} />
            </div>
          )}
        </label>
